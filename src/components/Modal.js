...
import {setUserStatus} from '../redux/slices/users/update-user';
...
export default function ConfirmationDialogRaw(props) {
  ...
  const { success,isLoading,error } = useSelector((state) => state.updateUser);
  ...
  const updateUserStatusFunc = async() => {
  const options = {id, actionType}
    dispatch(setUserStatus(options))
  }
      ...
      >
        <DialogContent dividers>
          <Paper elevation={0}>
            ...
            {success   && <div style={{color : 'green'}}> user status upadetd</div>}
        ...
        <DialogActions>
          <Box sx={{ width: '100%' }}>
            <Stack direction={'row'} sx={{ width: '100%' }} spacing={2}>
              ...
              <LoadingButton loading = {isLoading} variant="contained" onClick={updateUserStatusFunc}>
                proceed
              </LoadingButton>
            </Stack>
          </Box>
